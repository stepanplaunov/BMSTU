...
bodyTags = ["START_TASK", "END_TASK", "START_BODY", "END_BODY", "START_TESTS", "END_TESTS"]
import_reg = "[\\][\\$]IMPORT[\\][\\{]([A-Za-z0-9]*)[\\][\\}]"
system_reg = "[\\][\\$]SYSTEM[\\][\\_]([A-Za-z0-9]*)"

allTags = meta ++ bodyTags
...
getMetaData (tag:tags) (string:input) result =
 ...
 else filter ((/=0) . length) $ getSectionData bodyTags (splitOn " " $ intercalate " " (string:input)) result
 ...
 let
   generateLising ((b1, b2, _), (e1, e2, _)) =
    ...
    where
      ...
      (_, begin, _, atr) = (beginText =~ "{- BEGIN_COM KEY = {([A-Za-z0-9]*)} NAME = {([A-Za-z0-9]*)}( DESC = {([A-Za-z0-9\\ ]*)})?-}" :: (String, String, String, [String]))
      id = head atr
      name = atr !! 1
      desc = (atr++["",""]) !! 3
...
main = do
         ...
         let dat = lines labBmstu
             ...
             begins = map (add3 0) $ (getAllMatches (code =~ "{- BEGIN_COM KEY = {([A-Za-z0-9]*)} NAME = {([A-Za-z0-9]*)}( DESC = {([A-Za-z0-9\\ ]*)})?-}") :: [(Int, Int)])
             ends = map (add3 1) $ (getAllMatches (code =~ "{- END_COM-}") :: [(Int, Int)])
             add3 num (x, y) = (x, y, num)
             listings = sortOn (\(a,b,c) -> a) $ begins ++ ends
             tupfst (a, b, c) = a
             coords = commentCoords listings [] []
             checkComments = length coords == 0
             comments = getComments coords code
             lMap = Map.fromList $ map (\[a, b, c, d] -> (a, [b, withoutRules c, d])) comments
             beg = "{- BEGIN_COM KEY = {([A-Za-z0-9]*)} NAME = {([A-Za-z0-9]*)}-}"
             end = "{- END_COM-}"
...
--                        printer $ code !~ "{-BEGIN_COM KEY = {([0-9]*)} NAME = {([A-Za-z0-9]*)}-}{-END_COM-}-"
--                        print $ "aa..cqwcqww{}\n\raa" !~ "(.|[[:space:]])+"
--                        print lMap

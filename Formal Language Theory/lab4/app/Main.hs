...
deleteNullRules rules = addRightContext dln
 where
   ...
   --rulesCycle gra queue
   rulesForConst t pref (x:xs) constRules grammar = 0
    where
      newCR = constRules ++ [[t, pref, genNewT]]
      genNewT = 
   addRightContext grammar = (firstNames, firstRules)
    where
...
main = do
         ...
         then print $ id "Syntax error"
              ...
              printer $ deleteNullRules grammar

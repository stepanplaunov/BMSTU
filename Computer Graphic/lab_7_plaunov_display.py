...
def generate_list(dx, dy, dz, n, u):
    ...
    if id != 0:
        ...
        # result = pyglet.graphics.vertex_list(0)
        # result = (GLfloat * 3) (*[1, 1, 1])
        # texture_data = []
        ...
        if texture:
            ...
            for i in range(1, u + 1):
                ...
                for j in range(n):
                    a2, b2 = j / n * 1, (j + 1) / n * 1
                    glNormal3f(*vector(sub(data[0][(j + 1) % n], data[0][j]),
                                       sub([data[0][j][0] + dx, data[0][j][1] + dy, data[0][j][2] + dz], data[0][j])))
                    glTexCoord2d(a2, a)
                    glVertex3d(data[0][j][0] + dx * (i - 1) / u, data[0][j][1] + dy * (i - 1) / u,
                               data[0][j][2] + dz * (i - 1) / u)
                    glTexCoord2d(a2, b)
                    glVertex3d(data[0][j][0] + dx * i / u, data[0][j][1] + dy * i / u, data[0][j][2] + dz * i / u)
                glTexCoord2d(0, a)
                glVertex3d(data[0][0][0] + dx * (i - 1) / u, data[0][0][1] + dy * (i - 1) / u,
                           data[0][0][2] + dz * (i - 1) / u)
                glTexCoord2d(0, b)
            ...
            for j in range(len(data) - 1):
                ...
                for i in range(n + 1):
                    ...
                    if i != n:
                        glNormal3f(*vector(sub(data[j][(i + 1) % n], data[j][i]), sub(data[j + 1][i], data[j][i])))
                    glTexCoord2f(1 * cos(alpha) * (len(data) - j) / len(data) + 0.5,
                                 1 * sin(alpha) * (len(data) - j) / len(data) + 0.5)
                    glVertex3d(data[j][i][0] + dx, data[j][i][1] + dy, data[j][i][2] + dz)
                    glTexCoord2f(1 * cos(alpha) * (len(data) - j - 1) / len(data) + 0.5,
            ...
            for i in range(len(data) - 1):
                ...
                for j in range(n):
                    glNormal3f(
                        *[0, -1, 0])  # *vector(sub(data[i + 1][j], data[i][j]), sub(data[i][(j + 1) % n], data[i][j])))
                    glVertex3f(*data[i][j])
            ...
            for i in range(len(data) - 1):
                ...
                for j in range(n):
                    glNormal3f(*[0, 1,
                                 0])  # (*vector(sub(data[i][(j + 1) % n], data[i][j]), sub(data[i + 1][j], data[i][j]))))
...
def on_draw():
    ...
    if animation:
        if first_b:
            ...
            first_b = False
        animate(dt)
    draw_prizme(100, 400, 50, edge, side_polygon)
    # delty -= time.time()
    # delty = -delty
    # fps_data.append(delty)
    # print("MS: " + str(delta_time), 1000 / pyglet.clock.tick())
    if (pyglet.clock.get_default().last_ts % 5 < 0.01):
        # result = 0
        # for i in fps_data:
        #     result += i
        # result = (result / fps_size) * 1000
        # print(pyglet.clock.get_default().last_ts)
        print("MS: ", str(1000 / pyglet.clock.get_fps()))
        fps_data = []

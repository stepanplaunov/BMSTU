...
def draw_prizme(dx, dy, dz, n, u):
    global pos_prizme, saveid, savevec
    #glLoadIdentity()
    glTranslatef(*pos)
    glRotatef(pos_prizme[0], 1, 0, 0)
    glRotatef(pos_prizme[1], 0, 1, 0)
    glRotatef(pos_prizme[2], 0, 0, 1)
    if savevec != [texture, n, u]:
        savevec = [texture, n, u]
        generate_list(dx, dy, dz, n, u)
    glCallList(saveid)
    #glFlush()

def generate_list(dx, dy, dz, n, u):
    global saveid

    id = glGenLists(1)
    saveid = id
    print(id, dx, dy, dz, n, u)
    if id != 0:
        result = []
        texture_data = []
        normals = []
        ...
        if texture:
            for j in range(len(data) - 1):
                ...
                for i in range(n + 1):
                    if i != n:
                        normals += [*vector(sub(data[j][(i + 1) % n], data[j][i]), sub(data[j + 1][i], data[j][i]))]
                    i = i % n
                    alpha = 2 * i * pi / (n)

                    texture_data += [0.5 * cos(alpha) * (u - j) / u + 0.5, 0.5 * sin(alpha) * (u - j) / u + 0.5]
                    result += data[j][i]
                    result += data[j + 1][i]
                    texture_data += [0.5 * cos(alpha) * (u - j - 1) / u + 0.5, 0.5 * sin(alpha) * (u - j - 1) / u + 0.5]
            ...
            for i in range(1, u + 1):
                ...
                # print(u)
                for j in range(n):
                    a2, b2 = j / n * 10, (j + 1) / n * 10
                    normals += [*vector(sub(data[0][(j + 1) % n], data[0][j]),
                                       sub([data[0][j][0] + dx, data[0][j][1] + dy, data[0][j][2] + dz], data[0][j]))]
                    texture_data += [a2, a]
                    result += [data[0][j][0] + dx * (i - 1) / u, data[0][j][1] + dy * (i - 1) / u,
                               data[0][j][2] + dz * (i - 1) / u]
                    texture_data += [a2, b]
                    result += [data[0][j][0] + dx * i / u, data[0][j][1] + dy * i / u, data[0][j][2] + dz * i / u]
                texture_data += [0, a]
                result += [data[0][0][0] + dx * (i - 1) / u, data[0][0][1] + dy * (i - 1) / u,
                           data[0][0][2] + dz * (i - 1) / u]
                texture_data += [0, b]
                result += [data[0][0][0] + dx * i / u, data[0][0][1] + dy * i / u, data[0][0][2] + dz * i / u]
            ...
            for j in range(len(data) - 1):
                for i in range(n + 1):
                    ...
                    if i != n:
                        normals += [*vector(sub(data[j][(i + 1) % n], data[j][i]), sub(data[j + 1][i], data[j][i]))]
                    texture_data += [1 * cos(alpha) * (len(data) - j) / len(data) + 0.5, 1 * sin(alpha) * (len(data) - j) / len(data) + 0.5]
                    result += [data[j][i][0] + dx, data[j][i][1] + dy, data[j][i][2] + dz]
                    texture_data += [1 * cos(alpha) * (len(data) - j - 1) / len(data) + 0.5,
                                 1 * sin(alpha) * (len(data) - j - 1) / len(data) + 0.5]
                    result += [data[j + 1][i][0] + dx, data[j + 1][i][1] + dy, data[j + 1][i][2] + dz]

            result = (GLfloat * len(result))(*result)
            normals = (GLfloat * len(normals))(*normals)
            texture_data = (GLfloat * len(texture_data))(*texture_data)
            glEnableClientState(GL_TEXTURE_COORD_ARRAY_EXT)
            glEnableClientState(GL_VERTEX_ARRAY)
            glEnableClientState(GL_NORMAL_ARRAY)

            glVertexPointer(3, GL_FLOAT, 0, result)
            if texture:
                glTexCoordPointer(2, GL_FLOAT, 0, texture_data)
            if light:
                glNormalPointer(GL_FLOAT, 0, normals)

            glDrawArrays(GL_QUAD_STRIP, 0, len(result) // 3)

            glDisableClientState(GL_TEXTURE_COORD_ARRAY_EXT)
            glDisableClientState(GL_VERTEX_ARRAY)
            #glDrawArrays(GL_QUAD_STRIP, 0, len(texture_data) // 2)
            #glDrawArrays(GL_QUAD_STRIP, 0, len(result) // 3)
        else:
            ...
            for i in range(1, u + 1):
                ...
                for j in range(n):
                    ...
                    glVertex3f(data[0][j][0] + dx * (i - 1) / u, data[0][j][1] + dy * (i - 1) / u,
                               data[0][j][2] + dz * (i - 1) / u)
                    glVertex3f(data[0][j][0] + dx * i / u, data[0][j][1] + dy * i / u, data[0][j][2] + dz * i / u)
                glVertex3f(data[0][0][0] + dx * (i - 1) / u, data[0][0][1] + dy * (i - 1) / u,
                           data[0][0][2] + dz * (i - 1) / u)
...
class light(object):
    def __init__(self):
        ...
        self.position_light = [-50, 650.0, 250, 1.0]
...
@wind.event
def on_draw():
    global pos, edge, t_start, first_b, dt, fps_data,fps_size
    #delty = time.time()
    pyglet.clock.get_default().update_time()
    pyglet.clock.get_default()
    # print(pyglet.clock.get_default().last_ts)
    if texture:
        glEnable(tex.target)
    else:
    ...
    uz = degrees(asin(z / (2 - z ** 2) ** 0.5))#1949 гонка вооружений перешла в холодную войну
    #print(ux, uz)
    ...
    if animation:
        if first_b:
            ...
            first_b = False
        animate(dt)
    draw_prizme(10000, 4000, 50, edge+100, side_polygon)
    #delty -= time.time()
    #delty = -delty
    #fps_data.append(delty)
    #print("MS: " + str(delta_time), 1000 / pyglet.clock.tick())

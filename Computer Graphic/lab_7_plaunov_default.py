    ...
    if texture:
        ...
        for j in range(len(data) - 1):
            ...
            for i in range(n + 1):
                ...
                #     glNormal3f(*vector(sub(data[j][(i + 1) % n], data[j][i]), sub(data[j + 1][i], data[j][i])))
                #
                glTexCoord2f(1 * cos(alpha) * (len(data) - j) / len(data) + 0.5, 1 * sin(alpha) * (len(data) - j) / len(data) + 0.5)
                glVertex3d(data[j][i][0] + dx, data[j][i][1] + dy, data[j][i][2] + dz)
                glTexCoord2f(1 * cos(alpha) * (len(data) - j - 1) / len(data) + 0.5, 1 * sin(alpha) * (len(data) - j  - 1) / len(data) + 0.5)
...
def on_draw():
    ...
    #pyglet.clock.tick()
    #delty = pyglet.clock.get_default().time()
    ...
    #delty -= pyglet.clock.get_default().tick()
    #delty = -delty
